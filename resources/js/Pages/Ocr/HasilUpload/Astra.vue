...
const title = 'Hasil Upload Astra';
const page = usePage();
...
const getData = async (pageNumber = 1) => {
    ...
    try {
        ...
        console.error('Gagal mengambil data Astra:', error);
    } finally {
        loading.value = false;
    }
};
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-bold text-2xl text-[#1E293B] tracking-tight">
                Hasil Upload <span class="text-[#2DD4BF]">Mesin ASTRA</span>
            </h2>
        </template>
            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <div class="bg-white p-6 shadow-sm sm:rounded-3xl border border-slate-200">
                    <div class="mb-6 flex items-center justify-between">
                        <div>
                            <h3 class="text-lg font-black text-[#1E293B] uppercase tracking-tight">
                                Data Billing Information Mesin Astra
                            </h3>
                            <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">
                                Serial Number, Color, Black, dan Total Impressions (Astragraphia)
                            </p>
                        </div>
                    ...
                    <div v-else class="grid gap-5">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                        ...
                                        <div class="rounded-2xl bg-[#2DD4BF]/10 p-4 border border-[#2DD4BF]/20">
                                            ...
                                            <div class="mt-1 text-lg font-black text-[#1E293B]">
                                                {{ formatValue(getValue(item, 'total_impressions', 0)) }}
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="rounded-2xl border border-slate-200 p-4">
                                            ...
                                            <div class="space-y-2">
                                                <div class="flex justify-between gap-4 rounded-xl bg-emerald-50 p-3 text-sm">
                                                    <span class="font-bold text-emerald-600">Total Impressions</span>
                                                ...
                                                <div class="flex justify-between gap-4 rounded-xl bg-slate-50 p-3 text-sm">
                                                    <span class="font-bold text-slate-500">Color Impressions</span>
                                                    <span class="font-black text-[#1E293B] text-right">
                                                        {{ formatValue(getValue(item, 'color_impressions', 0)) }}
                                                    </span>
                                                </div>

                                                <div class="flex justify-between gap-4 rounded-xl bg-slate-50 p-3 text-sm">
                                                    <span class="font-bold text-slate-500">Color Large Impressions</span>
                                                    <span class="font-black text-[#1E293B] text-right">
                                                        {{ formatValue(getValue(item, 'color_large_impressions', 0)) }}
                                                    </span>
                                                </div>

                                                <div class="flex justify-between gap-4 rounded-xl bg-slate-50 p-3 text-sm">
                                                    <span class="font-bold text-slate-500">Black Impressions</span>
                                                    <span class="font-black text-[#1E293B] text-right">
                                                        {{ formatValue(getValue(item, 'black_impressions', 0)) }}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        >
                            Belum ada data hasil upload Mesin Astra.
                        </div>
