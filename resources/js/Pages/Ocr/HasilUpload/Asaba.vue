...
const title = 'Hasil Upload Asaba';
const page = usePage();
...
const getData = async (pageNumber = 1) => {
    ...
    try {
        ...
        console.error('Gagal mengambil data Asaba:', error);
    } finally {
        loading.value = false;
    }
};
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-bold text-2xl text-[#1E293B] tracking-tight">
                Hasil Upload <span class="text-[#2DD4BF]">Develop / Asaba</span>
            </h2>
        </template>
                ...
                <div class="bg-white p-6 shadow-sm sm:rounded-3xl border border-slate-200">
                    <div class="mb-6 flex items-center justify-between">
                        <div>
                            <h3 class="text-lg font-black text-[#1E293B] uppercase tracking-tight">
                                Data Counter Mesin Develop / Asaba
                            </h3>
                            <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">
                                Total, Printer, Copy, Scan, Feed, Output, Color, dan Black Counter
                            </p>
                        </div>
                    ...
                    <div class="mb-6 grid grid-cols-1 md:grid-cols-6 gap-3">
                        <input
                            ...
                            type="text"
                            placeholder="Cari mesin, serial, user, cabang..."
                    ...
                    <div v-else class="grid gap-5">
                        ...
                        >
                            <div class="flex flex-col gap-5 lg:flex-row">
                                ...
                                <div class="flex-1 space-y-5">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                        ...
                                        <div class="rounded-2xl bg-[#2DD4BF]/10 p-4 border border-[#2DD4BF]/20">
                                            ...
                                            <div class="mt-1 text-lg font-black text-[#1E293B]">
                                                {{ formatValue(getValue(item, 'total_counter', 0)) }}
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        <div class="rounded-2xl border border-slate-200 p-4">
                                            ...
                                            <div class="space-y-2">
                                                <div class="flex justify-between gap-4 rounded-xl bg-emerald-50 p-3 text-sm">
                                                    <span class="font-bold text-emerald-600">Total Counter</span>
                                                ...
                                                <div class="flex justify-between gap-4 rounded-xl bg-slate-50 p-3 text-sm">
                                                    <span class="font-bold text-slate-500">Printer Counter</span>
                                                    <span class="font-black text-[#1E293B] text-right">
                                                        {{ formatValue(getValue(item, 'printer_counter', 0)) }}
                                                    </span>
                                                </div>

                                                <div class="flex justify-between gap-4 rounded-xl bg-slate-50 p-3 text-sm">
                                                    <span class="font-bold text-slate-500">Copy Counter</span>
                                                    <span class="font-black text-[#1E293B] text-right">
                                                ...
                                                <div class="flex justify-between gap-4 rounded-xl bg-slate-50 p-3 text-sm">
                                                    <span class="font-bold text-slate-500">Scan Counter</span>
                                                    <span class="font-black text-[#1E293B] text-right">
                                                        {{ formatValue(getValue(item, 'scan_counter', 0)) }}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div class="rounded-2xl border border-slate-200 p-4">
                                        <h4 class="mb-3 text-[11px] font-black uppercase tracking-widest text-slate-500">
                                            Detail Counter Tambahan
                                        </h4>

                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                            <div class="flex justify-between gap-4 rounded-xl bg-slate-50 p-3 text-sm">
                                                <span class="font-bold text-slate-500">Feed Paper Counter</span>
                                                <span class="font-black text-[#1E293B] text-right">
                                                    {{ formatValue(getValue(item, 'feed_paper_counter', 0)) }}
                                                </span>
                                            </div>

                                            <div class="flex justify-between gap-4 rounded-xl bg-slate-50 p-3 text-sm">
                                                <span class="font-bold text-slate-500">Output Paper Counter</span>
                                                <span class="font-black text-[#1E293B] text-right">
                                                    {{ formatValue(getValue(item, 'output_paper_counter', 0)) }}
                                                </span>
                                            </div>

                                            <div class="flex justify-between gap-4 rounded-xl bg-slate-50 p-3 text-sm">
                                                <span class="font-bold text-slate-500">Full Color Counter</span>
                                                <span class="font-black text-[#1E293B] text-right">
                                                    {{ formatValue(getValue(item, 'full_color_counter', 0)) }}
                                                </span>
                                            </div>

                                            <div class="flex justify-between gap-4 rounded-xl bg-slate-50 p-3 text-sm">
                                                <span class="font-bold text-slate-500">Single Color Counter</span>
                                                <span class="font-black text-[#1E293B] text-right">
                                                    {{ formatValue(getValue(item, 'single_color_counter', 0)) }}
                                                </span>
                                            </div>

                                            <div class="flex justify-between gap-4 rounded-xl bg-slate-50 p-3 text-sm md:col-span-2">
                                                <span class="font-bold text-slate-500">Black Counter</span>
                                                <span class="font-black text-[#1E293B] text-right">
                                                    {{ formatValue(getValue(item, 'black_counter', 0)) }}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        >
                            Belum ada data hasil upload Develop / Asaba.
                        </div>
