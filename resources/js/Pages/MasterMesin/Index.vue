...
<template>
    ...
    <AuthenticatedLayout>
            ...
            <div class="flex items-center justify-between gap-4 bg-white p-4 rounded-3xl border border-slate-200 shadow-sm">
                <div class="flex flex-1 gap-4 max-w-2xl">
                    <div class="relative flex-1">
                        ...
                        <input v-model="search" type="text" placeholder="Cari Kode / Nama Mesin..."
            ...
            <div class="bg-white rounded-[2rem] border border-slate-200 overflow-hidden shadow-sm">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50 border-b border-slate-100">
                                <th class="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Kode Mesin</th>
                                <th class="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Nama & Merk</th>
                                <th class="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">S/N</th>
                                <th class="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Cabang</th>
                                <th class="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Harga Normal</th>
                        ...
                        <tbody class="divide-y divide-slate-50">
                            <template v-if="mesins.data.length > 0">
                                <tr v-for="item in mesins.data" :key="item.id" class="group hover:bg-slate-50/50 transition-colors">
                                    <td class="px-6 py-4">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-bold text-[#1E293B] uppercase tracking-tight">{{ item.nama_mesin }}</div>
                                        <div class="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{{ item.merk }} - {{ item.tipe }}</div>
                                    </td>
                                    <td class="px-6 py-4 text-[11px] font-black text-slate-500 uppercase">{{ item.serial_number || '-' }}</td>
                                    <td class="px-6 py-4 text-[11px] font-black text-slate-600 uppercase">{{ item.cabang?.nama_cabang || '-' }}</td>
                                    <td class="px-6 py-4 text-right">
                                        <div class="text-[11px] font-black text-[#1E293B]">{{ formatCurrency(item.harga_normal) }}</div>
                                        <div class="text-[8px] font-black text-emerald-500 uppercase">Min: {{ formatCurrency(item.harga_minimum) }}</div>
                                    </td>
                            ...
                            <tr v-else>
                                <td colspan="7" class="px-6 py-20 text-center">
                                    <h4 class="text-[13px] font-black text-[#1E293B] uppercase italic tracking-tighter">No Data <span class="text-[#2DD4BF]">Mesin</span> Found</h4>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div class="px-6 py-5 bg-slate-50/80 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em]">
                        Showing <span class="text-[#1E293B]">{{ mesins.from || 0 }}</span> to <span class="text-[#1E293B]">{{ mesins.to || 0 }}</span> of <span class="text-[#2DD4BF]">{{ mesins.total || 0 }}</span> Units
                    </div>
            ...
            <div class="bg-white rounded-[2rem] border border-slate-200 w-full max-w-2xl p-8 shadow-2xl relative max-h-[90vh] overflow-y-auto">
                <div class="flex justify-between items-center mb-8 border-b border-slate-100 pb-4 text-slate-800">
                    <h3 class="text-xl font-black uppercase italic tracking-tighter">
                        {{ isEdit ? 'Modify' : 'Register' }} <span class="text-[#2DD4BF]">Machine</span>
                    </h3>
                ...
                <form @submit.prevent="submit" class="space-y-5">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Kode Mesin</label>
                            <input v-model="form.kode_mesin" type="text" class="w-full px-4 py-2 bg-slate-50 border-none rounded-xl text-sm font-bold uppercase focus:ring-2 focus:ring-[#2DD4BF]/20" />
                        </div>
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nama Mesin</label>
                            <input v-model="form.nama_mesin" type="text" class="w-full px-4 py-2 bg-slate-50 border-none rounded-xl text-sm font-bold focus:ring-2 focus:ring-[#2DD4BF]/20" />
                        </div>
                    </div>

                    <div class="grid grid-cols-3 gap-4">
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Merk</label>
                            <input v-model="form.merk" type="text" class="w-full px-4 py-2 bg-slate-50 border-none rounded-xl text-sm font-bold focus:ring-2 focus:ring-[#2DD4BF]/20" />
                        </div>
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Tipe</label>
                            <input v-model="form.tipe" type="text" class="w-full px-4 py-2 bg-slate-50 border-none rounded-xl text-sm font-bold focus:ring-2 focus:ring-[#2DD4BF]/20" />
                        </div>
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Serial Number</label>
                            <input v-model="form.serial_number" type="text" class="w-full px-4 py-2 bg-slate-50 border-none rounded-xl text-sm font-bold focus:ring-2 focus:ring-[#2DD4BF]/20" />
                        </div>
                    </div>

                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Harga Minimum</label>
                            <input v-model="form.harga_minimum" type="number" class="w-full px-4 py-2 bg-slate-50 border-none rounded-xl text-sm font-bold focus:ring-2 focus:ring-[#2DD4BF]/20" />
                        </div>
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Harga Normal</label>
                            <input v-model="form.harga_normal" type="number" class="w-full px-4 py-2 bg-slate-50 border-none rounded-xl text-sm font-bold focus:ring-2 focus:ring-[#2DD4BF]/20" />
                        </div>
                    </div>
                    ...
                    <button type="submit" :disabled="form.processing"
                        class="w-full py-4 bg-[#2DD4BF] text-white rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] shadow-lg shadow-[#2DD4BF]/20 hover:bg-[#4f46e5] transition-all">
                        {{ form.processing ? 'Saving...' : 'Execute Machine Data' }}
                    </button>
                </form>
            </div>
        </div>
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
            <div class="bg-white rounded-[2rem] border border-slate-200 w-full max-sm p-8 shadow-2xl text-center">
                ...
                <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-8">Data mesin ini akan dihapus permanen.</p>
                <div class="flex gap-3">
                    ...
                    <button @click="executeDelete" :disabled="isDeleting" class="flex-1 py-3 bg-rose-500 text-white rounded-xl text-[10px] font-black uppercase tracking-widest">Hapus</button>
                </div>
            </div>
        </div>
    </AuthenticatedLayout>
