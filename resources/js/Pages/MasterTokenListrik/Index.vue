    ...
    <AuthenticatedLayout>
            ...
            <div class="bg-white rounded-[2rem] border border-slate-200 overflow-hidden shadow-sm">
                            ...
                            <tr v-else>
                                <td colspan="6" class="px-6 py-20 text-center">
                                    <h4 class="text-[13px] font-black text-[#1E293B] uppercase italic tracking-tighter">
                                        No <span class="text-[#2DD4BF]">Token Data</span> Found</h4>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div
                    ...
                    <div class="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em]">
                        Showing <span class="text-[#1E293B]">{{ tokenListriks.from || 0 }}</span> to <span
                            class="text-[#1E293B]">{{ tokenListriks.to || 0 }}</span> of <span class="text-[#2DD4BF]">{{
                            tokenListriks.total || 0 }}</span> Units
                    </div>
                    <nav v-if="tokenListriks.links && tokenListriks.links.length > 0" class="flex flex-wrap gap-1.5">
                        <template v-for="(link, k) in tokenListriks.links" :key="k">
                            ...
                            <Link v-else :href="link.url"
                                ...
                                :class="{ 'bg-[#1E293B] text-white border-[#1E293B] shadow-lg shadow-black/10 scale-105 z-10': link.active, 'bg-white text-slate-600 border-slate-200 hover:border-[#2DD4BF] hover:text-[#2DD4BF]': !link.active }"
        ...
        <div v-if="showModal"
            ...
            <div
                ...
                <form @submit.prevent="submit" class="space-y-5">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nomor
                                Meter</label>
                            <input v-model="form.nomor_meter" type="text" placeholder="Contoh: 1403..."
                                class="w-full px-4 py-2 bg-slate-50 border-none rounded-xl text-sm font-bold focus:ring-2 focus:ring-[#2DD4BF]/20" />
                            <div class="text-[9px] font-bold text-rose-500 uppercase mt-1 ml-1"
                                v-if="form.errors.nomor_meter">
                    ...
                    <div class="space-y-1.5">
                        <label
                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Keterangan</label>
                        <textarea v-model="form.keterangan" rows="2"
                            class="w-full px-4 py-2 bg-slate-50 border-none rounded-xl text-sm font-bold focus:ring-2 focus:ring-[#2DD4BF]/20"></textarea>
                    </div>
        ...
        <div v-if="showDeleteModal"
            ...
            <div class="bg-white rounded-[2rem] border border-slate-200 w-full max-w-sm p-8 shadow-2xl text-center">
                ...
                <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-8">Data token ini akan
                    dihapus
