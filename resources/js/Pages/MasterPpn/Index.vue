...
const openEdit = (item) => {
    ...
    form.nama_pajak = item.nama_pajak || '';
    form.persentase = item.persentase || 0;
...
const submit = () => {
    if (isEdit.value) {
        form.put(route('master-ppn.update', selectedId.value), {
            preserveScroll: true,
            onSuccess: () => closeModal(),
        });
    } else {
        form.post(route('master-ppn.store'), {
            preserveScroll: true,
            onSuccess: () => closeModal(),
        });
    }
...
const executeDelete = () => {
    ...
    router.delete(route('master-ppn.destroy', idToDelete.value), {
        preserveScroll: true,
        onSuccess: () => {
...
<template>
    <Head title="Master PPN" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-bold text-2xl text-[#1E293B] tracking-tight">
                Master <span class="text-[#2DD4BF]">PPN</span>
            </h2>
        </template>
            ...
            <div class="flex items-center justify-between gap-4 bg-white p-4 rounded-3xl border border-slate-200 shadow-sm">
                <div class="relative flex-1 max-w-xl">
                    ...
                    <input
                        v-model="search"
                        type="text"
                        placeholder="Cari nama pajak / keterangan..."
                ...
                >
                    <span class="mr-2 text-sm">+</span> Tambah PPN
                </button>
            </div>
            ...
            <div class="bg-white rounded-[2rem] border border-slate-200 overflow-hidden shadow-sm">
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-50">
                            <template v-if="ppns.data.length > 0">
                                <tr
                                    v-for="item in ppns.data"
                                    :key="item.id"
                            ...
                            <tr v-else>
                                <td colspan="5" class="px-6 py-20 text-center">
                                    <h4 class="text-[13px] font-black text-[#1E293B] uppercase italic tracking-tighter">
                                        No <span class="text-[#2DD4BF]">PPN Data</span> Found
                                    </h4>
                                </td>
                            </tr>
                        </tbody>
                ...
                <div class="px-6 py-5 bg-slate-50/80 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em]">
                        Showing
                        <span class="text-[#1E293B]">{{ ppns.from || 0 }}</span>
                        to
                        <span class="text-[#1E293B]">{{ ppns.to || 0 }}</span>
                        of
                        <span class="text-[#2DD4BF]">{{ ppns.total || 0 }}</span>
                        Data
                    </div>

                    <nav v-if="ppns.links && ppns.links.length > 0" class="flex flex-wrap gap-1.5">
                        <template v-for="(link, k) in ppns.links" :key="k">
                            <div
                                v-if="link.url === null"
            ...
            <div class="bg-white rounded-[2rem] border border-slate-200 w-full max-w-xl p-8 shadow-2xl relative max-h-[90vh] overflow-y-auto">
                <div class="flex justify-between items-center mb-8 border-b border-slate-100 pb-4 text-slate-800">
                    <h3 class="text-xl font-black uppercase italic tracking-tighter">
                        ...
                        <span class="text-[#2DD4BF]">PPN</span>
                    </h3>
                ...
                <form @submit.prevent="submit" class="space-y-5">
                    <div class="space-y-1.5">
                        ...
                        <div v-if="form.errors.nama_pajak" class="text-[10px] font-bold text-rose-500">
                            {{ form.errors.nama_pajak }}
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div v-if="form.errors.persentase" class="text-[10px] font-bold text-rose-500">
                            {{ form.errors.persentase }}
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div v-if="form.errors.keterangan" class="text-[10px] font-bold text-rose-500">
                    ...
                    >
                        {{ form.processing ? 'Processing...' : 'Execute PPN Data' }}
                    </button>
                </form>
            </div>
        </div>
        ...
        >
            <div class="bg-white rounded-[2rem] border border-slate-200 w-full max-w-sm p-8 shadow-2xl text-center">
                ...
                <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-8">
                    Data PPN ini akan dihapus permanen dari sistem.
                </p>
